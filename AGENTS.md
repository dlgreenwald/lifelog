...
GitHub Actions runs the verification pipeline on every PR and on every push to `main`. Four workflows live under `.github/workflows/`:
...
**Docker publishing** uses GitHub Container Registry (`ghcr.io`) with the auto-provisioned `GITHUB_TOKEN`. No `DOCKER_USERNAME` / `DOCKER_PASSWORD` secrets are required. `main.yml`'s `docker` job grants `packages: write` to `GITHUB_TOKEN` and pushes per-service images at `:main-${short_sha}` (sha-pinned). The `:latest` tag is owned by `containers.yml`: it gets applied via `workflow_call` from main.yml's `tag-latest` job (post-merge), or directly on a `containers-vX.Y.Z` tag push (coordinated release). Tagged releases (`container-vX.Y.Z`) also push `:X.Y.Z` and `<sha>` tags. Packages are private by default; promote to public later from each package's settings if needed. `build-and-test` and `security` don't touch the registry.
