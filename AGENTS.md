...
GitHub Actions runs the verification pipeline on every PR and on every push to `main`. Five workflows live under `.github/workflows/`:
...
- `main.yml` — main-branch gate (`on: push: branches: [main]`, exclusively — a push to `feature/*` does not schedule this workflow). Runs a `paths` job (same `dorny/paths-filter@v3` configuration as `ci.yml`'s, including the `ci-config` filter for `.github/workflows/**`, `AGENTS.md`, and `README.md`, and the `python-source` filter for default-branch CodeQL; filters are passed as a JSON-string input rather than a YAML block scalar to keep the runner loader consistent), then a `build-and-test` matrix of all six components — each gated per-component against the matching `paths` output so unrelated pushes skip irrelevant work. The `docker` matrix builds+pushes `server`/`speaker-id`/`transcription-worker`/`dashboard` images to ghcr.io tagged `:main-${short_sha}` (sha-pinned), each gated per service against its `paths` output (a firmware-only push skips docker entirely). The `tag-latest` job runs after `docker` and pulls each per-service `:main-${short_sha}` image, retags it as `:latest`, and pushes — this is the **sole writer of `:latest` on the post-merge path**, inlined into `main.yml` rather than dispatching through `containers.yml` via `uses:` (the runner dropped the caller workflow with `workflow_jobs_total_count = 0` whenever a job had `needs: <matrix-strategy>` + `if: success()` + `uses: <reusable>` together, so we sidestep `uses:` here). The `firmware-build` job runs `pio run -e xiao_esp32s3` and uploads `lifelog-firmware` + `lifelog-firmware-checksums` artifacts (30-day retention), gated on `paths.outputs.firmware`. The `security` job runs CodeQL `init`/`analyze` for `python,javascript` plus `npm audit --omit=dev --audit-level=critical` against the dashboard, gated on `paths.outputs.python-source || paths.outputs.dashboard || paths.outputs.ci-config`. Every downstream gate (`build-and-test`, `docker`, `tag-latest`, `firmware-build`, `security`) OR's in `ci-config` so a CI-file edit triggers a full re-validate. Firmware and container releases are cut by `release.yml` and `containers.yml` respectively, not `main.yml`.
- `containers.yml` — tag-push + workflow_call workflow for container image releases. Triggered by `containers-vX.Y.Z` tags: builds each of the four service images (`server`, `speaker-id`, `transcription-worker`, `dashboard`) from the tag's commit and pushes them to ghcr.io with tags `:X.Y.Z`, `<sha>`, and `:latest` (plus `:rc-N` if the tag carries an `-rcN` suffix). Also exposes a `workflow_call` trigger plus a `retag-latest` reusable job for callers wanting to pull `:main-${short_sha}` images and retag them as `:latest`. **Single owner of `:latest` on the tag-release path** — this workflow writes `:latest` for coordinated releases and provides a reusable `retag-latest` entry point; the post-merge path uses `main.yml`'s inline `tag-latest` job (same effect, sidesteps the `uses:` loader drop noted above). Full rebuild on every tag push (no path-gating inside this workflow) because a coordinated release means all four images share one version label.
...
**Docker publishing** uses GitHub Container Registry (`ghcr.io`) with the auto-provisioned `GITHUB_TOKEN`. No `DOCKER_USERNAME` / `DOCKER_PASSWORD` secrets are required. `main.yml`'s `docker` job grants `packages: write` to `GITHUB_TOKEN` and pushes per-service images at `:main-${short_sha}` (sha-pinned). After the matrix settles, `main.yml`'s `tag-latest` job pulls each `:main-${short_sha}` image, retags it as `:latest`, and pushes — the **sole writer of `:latest` on the post-merge path**. The `:latest` tag is also written by `containers.yml` on a coordinated `containers-vX.Y.Z` tag push (it then reads tag-of-the-moment, so it could be larger than the most recent `*-${short_sha}` if you only do tag releases). Tagged releases (`containers-vX.Y.Z`) also push `:X.Y.Z` and `<sha>` tags. Packages are private by default; promote to public later from each package's settings if needed. `build-and-test`, `firmware-build`, and `security` don't touch the registry.
