...
- `main-server.yml` — main-branch gate for the server image. Trigger: `on.push.branches: [main]` with `paths: [server/**, .github/workflows/**, AGENTS.md]`. Runs the server `build.sh` via `pr-check.yml` (uv venv, ruff, pytest with coverage); on success, builds the docker image and pushes `ghcr.io/<owner>/lifelog-server:main-${short_sha}` (sha-pinned).
- `main-speaker-id.yml` — main-branch gate for the speaker-id image. Same shape as `main-server.yml`, gated on `speaker-id/**` paths; pushes `lifelog-speaker-id:main-${short_sha}`.
- `main-transcription-worker.yml` — main-branch gate for the transcription-worker image. Same shape, gated on `transcription-worker/**` paths; pushes `lifelog-transcription-worker:main-${short_sha}`.
- `main-dashboard.yml` — main-branch gate for the dashboard image. Same shape, gated on `dashboard/**` paths; pushes `lifelog-dashboard:main-${short_sha}`.
- `main-firmware.yml` — main-branch gate for firmware-ota. Same path-filter shape, gated on `firmware-ota/**` paths. Re-runs `pio test -e test` via `pr-check.yml` plus an extra `pio run -e xiao_esp32s3` build step. **Not a container** — firmware is shipped via `release.yml` (no docker image).
- `main-tag-latest.yml` — sole **post-merge writer of the `:latest` tag** on ghcr.io. Trigger: `on.push.branches: [main]` with `paths: [server/**, speaker-id/**, transcription-worker/**, dashboard/**, .github/workflows/main-*.yml, .github/workflows/main-tag-latest.yml]`. A `retag-latest` matrix job walks the four services, pulls the just-published `:main-${short_sha}` image, retags as `:latest`, and pushes (`docker pull` failures log a warning and continue — a service that didn't publish this commit is skipped). The path filter covers every input the prior merge may have fed to the image publishers, so `:latest` always lands within the same push event.
- `containers.yml` — release-only tag-push workflow. Trigger: `on.push.tags: [containers-v*]`. On a `containers-vX.Y.Z` tag push, pulls `:latest` for each of the four services, retags as `:X.Y.Z` and `<sha>` (plus `:rc-N` if the tag carries an `-rcN` suffix), and pushes. No rebuild — the merge-contract image set is the source of truth. Does NOT touch `:latest`. (Earlier history: this file used to be the dual-writer of `:latest` and was also called via `workflow_call` from the post-merge surface; both modes were removed when the merge surface moved to `main-tag-latest.yml`. The release-only role keeps a single owner model and ships the version retag.)
- `release.yml` — tag-push + workflow_dispatch workflow. Triggers on tags matching `firmware-v*` (matching the existing `firmware-v0.1` convention) or via the Actions "Run workflow" button with an arbitrary tag string. Re-builds the firmware from the tag's source on the same Ubuntu runner, then attaches `firmware.bin`, `bootloader.bin`, `partitions.bin`, and `SHA256SUMS` to a GitHub Release via `softprops/action-gh-release@v2` (`contents: write` permission).
`Generate-release-notes` is enabled, so each release gets an auto-generated body from commits since the previous tag.

Partial-build verification: a PR touching only `server/` shows `detect-changes` plus the `server` and `security` jobs (because server source is Python and CodeQL/npm-audit run on PRs touching Python or `dashboard/`). Dashboard / firmware / diarization / speaker-id / transcription-worker nodes are skipped via the output gate. The `ci-config` filter short-circuits this: a PR touching only `.github/workflows/**`, `AGENTS.md`, or `README.md` runs every job in the matrix — that's intentional, since a path-filter or workflow change should not go unverified. On push to main, the same path rules apply via workflow-level `on.push.paths` filters, so a fingerprint-edit-only merge (e.g., `AGENTS.md`) only triggers `main-{server,speaker-id,transcription-worker,dashboard,firmware}.yml` (each carries `.github/workflows/**` and `AGENTS.md` in its gate list) and `main-tag-latest.yml` if any image source also changed. A pure-docs push fires the five component workflows because `.github/workflows/**` is in every paths list; this is deliberate — a workflow/AGENTS change must re-validate every merge contract.
...
**Docker publishing** uses GitHub Container Registry (`ghcr.io`) with the auto-provisioned `GITHUB_TOKEN`. No `DOCKER_USERNAME` / `DOCKER_PASSWORD` secrets are required. The four `main-<service>.yml` workflows grant `packages: write` to `GITHUB_TOKEN` and each push its image at `:main-${short_sha}` (sha-pinned). `main-tag-latest.yml` is the **sole post-merge writer of `:latest`** — it pulls the just-published `:main-${short_sha}` images, retags, and pushes. On a `containers-vX.Y.Z` tag push, `containers.yml` pulls `:latest` and retags to `:X.Y.Z` (and `<sha>`, `:rc-N` if applicable) without rebuilding. Tagged releases (`container-vX.Y.Z`) also push `:X.Y.Z` and `<sha>` tags. Packages are private by default; promote to public later from each package's settings if needed.
