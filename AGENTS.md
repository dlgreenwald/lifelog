...
- `main.yml` — main-branch gate. Runs a `paths` job (same `dorny/paths-filter@v3` configuration as `ci.yml`'s, including the `ci-config` filter for `.github/workflows/**`, `AGENTS.md`, and `README.md`), then a `build-and-test` matrix of all six components — each gated per-component against the matching `paths` output so unrelated pushes skip irrelevant work. The `docker` matrix builds+pushes `server`/`speaker-id`/`transcription-worker`/`dashboard` images to ghcr.io, each gated per service against its `paths` output. The `firmware-build` job runs `pio run -e xiao_esp32s3` and uploads `lifelog-firmware` + `lifelog-firmware-checksums` artifacts (30-day retention), gated on `paths.outputs.firmware`. Every downstream gate (`build-and-test`, `docker`, `firmware-build`) OR's in `ci-config` so a CI-file edit triggers a full re-validate. The `security` job used to duplicate `ci.yml`'s; it was removed so Code Scanning has a single workflow to map per-PR alerts against. Firmware releases are cut by `release.yml`, not `main.yml`.
