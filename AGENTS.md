...
- `main.yml` — main-branch gate. Drives `pr-check.yml` across all six build components via `matrix: [server, diarization, speaker-id, transcription-worker, dashboard, firmware]`, runs `docker` (matrix of `server`, `speaker-id`, `transcription-worker`) with `needs: build-and-test`, and runs a parallel `firmware-build` job that compiles `pio run -e xiao_esp32s3` and uploads `lifelog-firmware` + `lifelog-firmware-checksums` artifacts (30-day retention). The `security` job used to duplicate `ci.yml`'s; it was removed so Code Scanning has a single workflow to map per-PR alerts against. Firmware releases are cut by `release.yml`, not `main.yml`.
