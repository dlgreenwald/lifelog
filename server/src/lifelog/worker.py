...
async def process_utterance(user_id: int, utterance_id: int):
    ...
    if not chunks:
        logger.warning(
            "no_chunks_for_utterance", user_id=user_id, utterance_id=utterance_id
        )
    ...
    if active_session:
        ...
        if gap_minutes <= settings.session_gap_minutes:
            ...
            try:
                ...
                logger.exception(
                    "error_queuing_ended_session", session_id=active_session["id"]
                )
...
async def _create_session_quick_jobs() -> None:
    ...
    for session in sessions:
        ...
        try:
            ...
            logger.exception("error_creating_quick_job", session_id=session["id"])
...
async def _apply_quick_transcripts() -> None:
    ...
    for job in await db.get_completed_quick_jobs():
        try:
            ...
            logger.exception("unable_to_apply_quick_job", job_id=job.get("id"))


def _window_ranges(utterances: list[dict]) -> list[tuple[datetime, datetime]]:
    """Split a session's utterances into time windows for batched reprocessing."""
...
async def _reprocess_session(session: dict):
    ...
    utterances = await db.get_session_all_utterances(session_id)
    if not utterances:
        logger.warning("session_has_no_utterances", session_id=session_id)
    ...
    for chunk_index, (window_start, window_end) in enumerate(
        ...
        if job is not None:
            if job.get("status") == "failed":
                logger.error(
                    "session_chunk_has_terminal_failed_job",
                    session_id=session_id,
        ...
        logger.info(
            "full_transcription_chunk_queued",
            session_id=session_id,
            chunk_index=chunk_index,
        )
...
async def _auto_enroll_speakers(user: dict, speaker_segments: list[dict]) -> None:
    ...
    for label, audio_files in grouped.items():
        ...
        try:
            ...
            logger.exception("auto_enroll_failed", label=label)
...
async def _enroll_session_speakers(
    ...
    for label, opus_audio in all_speakers.items():
        try:
            ...
            logger.exception("enroll_from_partitions_failed", label=label)
...
async def _reidentify_recording(user: dict, recording: dict) -> None:
    ...
    if not segments:
        ...
        try:
            ...
            logger.exception(
                "reidentify_recording_failed", recording_id=recording.get("id")
            )
        return
    updated = []
    labels: dict[str, str] = {}
...
async def _finalize_completed_sessions() -> None:
    ...
    for session in sessions:
        try:
            ...
            if not partitions:
                logger.warning(
                    "session_has_no_speaker_segments",
                    session_id=session["id"],
                )
            ...
            if len(partitions) == 1:
                ...
                try:
                    ...
                    logger.exception(
                        "daily_summary_update_failed", session_id=session["id"]
                    )
            else:
                ...
                for idx, partition in enumerate(partitions[1:], start=1):
                    ...
                    try:
                        ...
                        logger.exception(
                            "partition_recording_creation_failed",
                            partition_idx=idx,
                            session_id=session["id"],
                        )
                ...
                try:
                    ...
                    logger.exception(
                        "daily_summary_update_failed",
                        session_id=session["id"],
                    )

        except Exception:
            logger.exception("finalize_session_failed", session_id=session["id"])


async def worker_loop():
    """Poll uploads, apply quick results, and orchestrate full jobs."""
    logger.info("worker_started", poll_interval_s=POLL_INTERVAL)
    while True:
        try:
            ...
            try:
                ...
                logger.exception("apply_quick_transcripts_failed")
            try:
                await _create_session_quick_jobs()
            except Exception:
                logger.exception("create_session_quick_jobs_failed")
            try:
                for session in await db.get_idle_active_sessions(
                    ...
                    try:
                        ...
                        logger.exception(
                            "queue_idle_session_failed", session_id=session["id"]
                        )
            except Exception:
                logger.exception("end_idle_sessions_failed")
            try:
                await _finalize_completed_sessions()
            except Exception:
                logger.exception("finalize_completed_sessions_failed")
        except Exception:
...
async def hourly_reprocess_loop():
    ...
    logger.info("hourly_reprocess_loop_started", interval_s=interval)
        ...
        try:
            ...
            for session in sessions:
                ...
                try:
                    ...
                    logger.exception(
                        "hourly_reprocess_session_failed", session_id=session["id"]
                    )
        except Exception:
...
async def _daily_reprocess_user(user_id: int, target_date=None, llm_context: str = ""):
    ...
    logger.info(
        ...
        user_id=user_id,
        target_date=str(target_date),
        summary_char_count=len(daily_summary),
    )
